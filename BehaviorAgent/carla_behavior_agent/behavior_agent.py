...
class BehaviorAgent(BasicAgent):
    ...
    def controlled_stop(self, vehicle=None, distance=0.0,minDistance=3.5):
        ...
        if vehicle != None:
            vehicle_speed = get_speed(vehicle)
        delta_v = max(1, (self._speed - vehicle_speed) / 3.6)
        ttc = (distance - minDistance) / delta_v if delta_v != 0 else distance / np.nextafter(0., 1.)
    ...
    def cond_to_start_surpass(self,ego_vehicle_wp):
            ...
            distance_to_surpass = 30 # DA VERIFICARE sarà lo spazio totale che dovrò sorpassare, lo settiamo in partenza di 15 per considerare un certo parametro di sicurezza
